...
int main(int argc, char **argv)
{
	...
	//Calculate max instert numbers of each page
	Count nattr = nattrs(r);
	Count c = PAGESIZE/(10*nattr);
	if (c*10*nattr < PAGESIZE) {
		c++;
	}
	...
	while ((t = readTuple(r,stdin)) != NULL) {
		...
		// Page pg = getPage(getData(r) ,pid);
		// if (pageTuples(pg) >= c && splitp(r) == pid) {
		// 	printf("Split Page!\n");
		// 	splitPage(r);
