...
{	
	...
	assert(vals != NULL);
	tupleVals(t, vals);
	// ./insert R with 100,abc,xyz Then vals[0] = '100',vals[1] = 'abc', and vals[2] = 'xyz'
	for (int i = 0; i < nvals; i++) {
		hash = hash_any((unsigned char *)vals[i],strlen(vals[i]));
		setupBits(hash,buf[i]);
		// printf("%s\n", buf[i]);
	}
	for (int i = 0;i < MAXCHVEC; i++) {
		hashValue[MAXCHVEC - i - 1] = buf[cv[i].att][MAXCHVEC - 1 - cv[i].bit];
	}
	hashValue[MAXCHVEC] = '\0';
	// printf("%s\n", hashValue);
	...
	for (int i = 0; i < MAXCHVEC; i++) {
		if (hashValue[i] == '0') {
			hash = unsetBit(hash, MAXCHVEC - 1 - i);
		} else {
			hash = setBit(hash, MAXCHVEC - 1 - i);
		}
	}
	// showBits(hash,buf[0]);
	// printf("hash(%s) = %s\n",t,buf[0]);
